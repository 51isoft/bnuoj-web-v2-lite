...
(function($){
	...
	var defaultOptions = {
		...
		animationSettings: {
			up: {
				left: -25, // Move left
				backgroundColor: '#66ff66' // Dullish green
			},
			down: {
				left: 25, // Move right
				backgroundColor: '#ff5555' // Dullish red
			},
			fresh: {
...
//        console.log('Begin Setup: '+elatime);
		...
		jOrigTable.find('div.moveable').each(function(i, wrapper){
			...
			if(column in updatingColumns && (column != positionColumn || !(row in stayPut))){
				...
				firstAnimator.addSubject(new NumericalStyleSubject(innerWrapper, "opacity", 1, 0.8, ""));
				if(newCell != null){
					thirdAnimator.addSubject(new NumericalStyleSubject(innerWrapper, "opacity", 0.8, 1, ""));
